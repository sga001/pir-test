...
use pir_test::test_sealpir::FutureServiceExt;
use pir_test::test_sealpir::PirTestServer;
use rand::RngCore;
...
fn main() {
    ...
    let server = PirTestServer::new(ele_num, ELE_SIZE as u32, POLY_DEGREE, log_plain_mod, d);
    ...
    let (_, conn) = server
        .listen(
            ...
            &reactor.handle(),
            server::Options::default().max_payload_size(1 << 24),
        )
        .unwrap();

    println!("ready at {}", server_addr);
